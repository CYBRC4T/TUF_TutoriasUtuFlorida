                ...
                <section class="tutorias">
                    ...
                    <div class="grid">
                        <?php
                            ...
                            $conn = mysqli_connect("localhost", "root", $dbb_con);
                            $db = mysqli_select_db($conn, "tutorias");
                            
                            $query = mysqli_query($conn, "SELECT tutorias.id_tut AS id_tut, materias.nombre AS materia FROM materias, tutorias WHERE materias.id_mat = materia LIMIT 4");

                            while($row = mysqli_fetch_assoc($query))
                            {
                                echo("<form class='item' action='detalleTut.php' method='post'> 
                                        <input type='hidden' name='id_tut' id='id_tut' value='" . $row['id_tut'] . "'> 
                                        <input type='submit' value='" . $row['materia'] . "'> 
                                    </form>");
                            }
                        ?>
                    </div>
