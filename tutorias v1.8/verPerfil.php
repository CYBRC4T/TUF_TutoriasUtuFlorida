    ...
    <div class="fullimgBG" onclick="changePFP()" id="pfpBg"></div>
        ...
        <div class="pfp">
            <?php
                ...
                if(file_exists("archivos/pfp/" . $_POST['perfCI'] . ".jpg")) {
                    echo("<img src='archivos/pfp/" . $_POST['perfCI'] . ".jpg' onclick='seeImage(this.getAttribute('src'))'/>");
                } else {
                    if(file_exists("archivos/pfp/" . $_POST['perfCI'] . ".jpeg")) {
                        echo("<img src='archivos/pfp/" . $_POST['perfCI'] . ".jpeg' onclick='seeImage(this.getAttribute('src'))'/>");
                    } else {
                        if(file_exists("archivos/pfp/" . $_POST['perfCI'] . ".png")) {
                            echo("<img src='archivos/pfp/" . $_POST['perfCI'] . ".png' onclick='seeImage(this.getAttribute('src'))'/>");
                        } else {
                            echo("<img src='archivos/pfp/pfpDefault.jpg' onclick='seeImage(this.getAttribute('src'))'/>");
                        }
                    }
                }
                ...
                {
                    echo("<form action='uploadManager.php' id='uploadForm' method='post' enctype='multipart/form-data'>");
                    echo("<label for='archivo'> CAMBIAR FOTO DE PERFIL </label>");
                    echo("<input type='file' name='archivo' id='archivo' accept='.jpg, .jpeg, .png' onchange='changePFP()'>");
                    echo("<input type='hidden' name='arcType' id='arcType' value='pfp'");
                    echo("</form>");
                }
            ?>
        </div>
        ...
        <div class="container" id="noBg">
            <div class="detalles">
                ...
                <div class="item" id="nombre">
                    ...
                    <?php 
                        ...
                        {
                            case 'EST':
                                $query = mysqli_query($sql, "SELECT nombre, apellido FROM estudiantes WHERE ci_est = " . $_COOKIE['ci']);
                                break;
                            case 'DOC':
                                $query = mysqli_query($sql, "SELECT nombre, apellido FROM docentes WHERE ci_doc = " . $_COOKIE['ci']);
                                break;
                            case 'ADM':
                                $query = mysqli_query($sql, "SELECT nombre, apellido FROM administradores WHERE ci_adm = " . $_COOKIE['ci']);
                                break;
                        }
                    ...
                    <table>
                        <?php
                            if($_COOKIE['rol'] == 'EST')
                            {
                                ...
                                for($x = 0; $x < 20; $x++)
                                {
                                    echo("<tr id='prof'> <td> 0 </td> <td> NOMBRE MATERIA </td> </tr>");
                                }
                            }
                        ?>
                    </table>
            ...
            <?php
                ...
                {
                    case 'EST':
                        echo("<a id='perfbtn' href='inicioEst.php'> VOLVER </a>");
                        break;
                    case 'DOC':
                        echo("<a id='perfbtn' href='inicioDoc.php'> VOLVER </a>");
                        break;
                    case 'ADM':
                        echo("<a id='perfbtn' href='inicioAdm.php'> VOLVER </a>");
                        break;
                }
            ?>
        </div>
